...
/**
 * Props:
 *   onSelect(analysisId) — called when user clicks a history item
 *   selectedId           — currently viewed analysis id (for highlight)
 *   onClose              — close sidebar callback
 *   compareIds           — array of ids selected for comparison
 *   onToggleCompare(id)  — toggle an id in/out of comparison
 */
export default function HistorySidebar({
  onSelect, selectedId, onClose, compareIds = [], onToggleCompare,
}) {
      ...
      <div className="flex items-center justify-between px-4 py-4 border-b border-gray-800">
        ...
        <div className="flex gap-2">
          <button onClick={fetchHistory}
            className="text-gray-500 hover:text-amber-400 transition-colors text-xs border border-gray-700 hover:border-amber-400 px-2 py-1 rounded">
            ↻
          </button>
          <button onClick={onClose}
            className="text-gray-500 hover:text-white transition-colors text-xs border border-gray-700 hover:border-gray-500 px-2 py-1 rounded">
            ✕
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && items.length === 0 && (
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-gray-700 text-xs mt-1">Run your first analysis to see history.</p>
          </div>
        )}
          ...
          return (
            <div key={item.id}
              className={`group border-b border-gray-800/60 transition-all ${
                isSelected ? "bg-amber-950/40" : "hover:bg-gray-900"
              }`}>
              <button
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-sm font-semibold truncate ${isSelected ? "text-amber-400" : "text-white"}`}>
                        {item.startup_name}
                      </span>
                    </div>

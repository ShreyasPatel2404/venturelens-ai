...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-xs shadow-xl">
      <p className="text-amber-400 font-bold">{d.dimension}</p>
      <p className="text-white font-mono text-base font-bold">
        {d.value}<span className="text-gray-400 text-xs">/100</span>
      </p>
    </div>
  );
};
  ...
  return (
    <div className="w-full">
      ...
      <ResponsiveContainer width="100%" height={height}>
        {/* ← data prop must be on RechartsRadar, not on Radar */}
